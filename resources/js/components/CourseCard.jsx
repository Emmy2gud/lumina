...
const CourseCard = ({ course, className }) => {
    const { post, processing, get } = useForm();
    ...
    return (
        <form
            onSubmit={HandleCartSubmit(course.id)}
            encType="multipart/form-data"
        >
            <Card
                className={cn(
                    "overflow-hidden card-hover bg-white border-0 shadow-lg",
                    className
                )}
            >
                ...
                <CardContent className="p-4">
                    <Link href={`/course/${course.id}/view`}>
                        ...
                        <div className="mt-2 flex items-center gap-1 text-xs">
                            <span className="text-sm text-gray-600 mb-4 line-clamp-2 ">
                                {course.description}
                            </span>
                        </div>
